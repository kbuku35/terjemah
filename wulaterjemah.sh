...
#	Author:	wulabing / di terjemahkan oleh djasri
...
chrony_install() {
    ...
    echo -e "${OK} ${GreenBG} Menunggu sinkronisasi waktu ${Font}"
    sleep 10

    chronyc sourcestats -v
    chronyc tracking -v
    date
    read -rp "Mohon konfirmasi apakah waktunya akurat, rentang kesalahan adalah ± 3 menit(Y/N): " chrony_install
...
port_alterid_set() {
    if [[ "on" != "$old_config_status" ]]; then
        read -rp "Harap masukkan port koneksi（default:443）:" port
        [[ -z ${port} ]] && port="443"
        read -rp "silahkan masukan alterID（default:2 Hanya nomor yang diperbolehkan）:" alterID
        [[ -z ${alterID} ]] && alterID="2"
    fi
}
modify_path() {
    ...
    judge "V2ray Modifikasi jalur kamuflase"
}
modify_alterid() {
    if [[ "on" == "$old_config_status" ]]; then
...
v2ray_install() {
    ...
    if [[ -f v2ray.sh ]]; then
        ...
        echo -e "${Error} ${RedBG} V2ray Pengunduhan file penginstalan gagal, periksa apakah alamat pengunduhan tersedia ${Font}"
        exit 4
    fi
    # 清除临时文件
    rm -rf /root/v2ray
...
nginx_exist_check() {
    if [[ -f "/etc/nginx/sbin/nginx" ]]; then
        echo -e "${OK} ${GreenBG} Nginx Sudah ada, lewati kompilasi dan instal ${Font}"
        sleep 2
    elif [[ -d "/usr/local/nginx/" ]]; then
        echo -e "${OK} ${GreenBG} Nginx yang diinstal oleh paket lain terdeteksi. Lanjutkan menginstal akan menyebabkan konflik. Silakan instal setelah menangani${Font}"
        exit 1
    else
        nginx_install
    fi
...
nginx_install() {
    ...
    echo -e "${OK} ${GreenBG} Akan mulai menyusun dan menginstal jemalloc ${Font}"
    sleep 2
    ...
    echo -e "${OK} ${GreenBG} Kompilasi dan instalasi Nginx akan segera dimulai, prosesnya akan memakan waktu sedikit lebih lama, harap bersabar ${Font}"
    sleep 4
...
ssl_install() {
    ...
    judge "安装 SSL Skrip pembuatan sertifikat bergantung pada"

    curl https://get.acme.sh | sh
    judge "安装 SSL Skrip pembuatan sertifikat"
}
domain_check() {
    read -rp "masukan nama domain anda(contoh:www.wulabing.com):" domain
    domain_ip=$(ping "${domain}" -c 1 | sed '1{s/[^(]*(//;s/).*//;q}')
    echo -e "${OK} ${GreenBG} Mendapatkan informasi ip jaringan publik, harap tunggu dengan sabar ${Font}"
    ...
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        ...
        echo -e "${Error} ${RedBG} Harap pastikan bahwa rekam A yang benar ditambahkan ke nama domain, jika tidak maka tidak akan berfungsi dengan baik V2ray ${Font}"
...
acme() {
    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force --test; then
        echo -e "${OK} ${GreenBG} SSL Tes sertifikat berhasil dikeluarkan dan penerbitan resmi dimulai ${Font}"
...
vmess_link_image_choice() {
        echo "Pilih jenis tautan yang dibuat"
...
basic_information() {
    {
        ...
        echo -e "${Red} V2ray Informasi konfigurasi ${Font}"
        echo -e "${Red} url（address）:${Font} $(info_extraction '\"add\"') "
        ...
        echo -e "${Red} security（security）：${Font} 自适应 "
        echo -e "${Red} network（network）：${Font} $(info_extraction '\"net\"') "
        echo -e "${Red} type（type）：${Font} none "
        echo -e "${Red} jalan（不要落下/）：${Font} $(info_extraction '\"path\"') "
        echo -e "${Red} Keamanan transmisi yang mendasari：${Font} tls "
    } >"${v2ray_info_file}"
}
show_information() {
    cat "${v2ray_info_file}"
...
ssl_judge_and_install() {
    if [[ -f "/data/v2ray.key" || -f "/data/v2ray.crt" ]]; then
        echo "/data File sertifikat di direktori sudah ada"
        echo -e "${OK} ${GreenBG} hapus atau tidak [Y/N]? ${Font}"
        read -r ssl_delete
        case $ssl_delete in
        [yY][eE][sS] | [yY])
            rm -rf /data/*
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo -e "${Error} ${RedBG} Nginx Atau file konfigurasi tidak ada atau versi yang saat ini diinstal adalah h2, silakan jalankan setelah menginstal script dengan benar${Font}"
    fi
}
show_access_log() {
    [ -f ${v2ray_access_log} ] && tail -f ${v2ray_access_log} || echo -e "${RedBG}logfile tidak ada${Font}"
}
show_error_log() {
    [ -f ${v2ray_error_log} ] && tail -f ${v2ray_error_log} || echo -e "${RedBG}logfile tidak ada${Font}"
}
ssl_update_manuel() {
    [ -f ${amce_sh_file} ] && "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" || echo -e "${RedBG}Alat penerbitan sertifikat tidak ada, harap konfirmasi apakah Anda telah menggunakan sertifikat Anda sendiri${Font}"
...
mtproxy_sh() {
    echo -e "${Error} ${RedBG} Pemeliharaan fungsi, sementara tidak tersedia ${Font}"
}
...
uninstall_all() {
    ...
    if [[ -d $nginx_dir ]]; then
        echo -e "${OK} ${Green} Apakah akan mencopot pemasangan Nginx [Y/N]? ${Font}"
        read -r uninstall_nginx
        case $uninstall_nginx in
        [yY][eE][sS] | [yY])
            rm -rf $nginx_dir
            echo -e "${OK} ${Green} Tidak terinstal Nginx ${Font}"
            ;;
        *) ;;
    ...
    systemctl daemon-reload
    echo -e "${OK} ${GreenBG} Telah dicopot, file sertifikat SSL telah dipertahankan ${Font}"
}
delete_tls_key_and_crt() {
    [[ -f $HOME/.acme.sh/acme.sh ]] && /root/.acme.sh/acme.sh uninstall >/dev/null 2>&1
    [[ -d $HOME/.acme.sh ]] && rm -rf "$HOME/.acme.sh"
    echo -e "${OK} ${GreenBG} File sertifikat yang tersisa telah dikosongkan ${Font}"
}
judge_mode() {
...
update_sh() {
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            ...
            echo -e "${OK} ${GreenBG} pembaruan selesai ${Font}"
            exit 0
            ;;
        *) ;;
        ...
        echo -e "${OK} ${GreenBG} Versi saat ini adalah versi terbaru ${Font}"
    fi

}
maintain() {
    echo -e "${RedBG}Opsi ini untuk sementara tidak tersedia${Font}"
    echo -e "${RedBG}$1${Font}"
    exit 0
}
list() {
...
menu() {
    ...
    echo -e "\t---authored by wulabing di terjemahkan oleh djas---"
    ...
    echo -e "${Green}2.${Font}  Install V2Ray (http/2:Khusus untuk IMCLASS)"
    echo -e "${Green}3.${Font}  Update V2Ray core"
    echo -e "—————————————— Ganti Konfigurasi ——————————————"
    echo -e "${Green}4.${Font}  Ganti UUID"
    echo -e "${Green}5.${Font}  Ganti alterid"
    echo -e "${Green}6.${Font}  Ganti port"
    echo -e "${Green}7.${Font}  Ganti Versi TLS (Jikaws+tlsValid)"
    echo -e "—————————————— Cek Informasi ——————————————"
    echo -e "${Green}8.${Font}  Lihat log akses Real-Time"
    echo -e "${Green}9.${Font}  Lihat log kesalahan Real-Time"
    echo -e "${Green}10.${Font} Lihat informasi konfigurasi V2Ray"
    echo -e "—————————————— Pilihan Lain ——————————————"
    echo -e "${Green}11.${Font} Install 4in1 BBR Untuk Meningkatkan Speed"
    echo -e "${Green}12.${Font} Instal MTproxy (mendukung obfuscation TLS)"
    echo -e "${Green}13.${Font} Perbaharui Sertifikat yang Kadaluarsa"
    echo -e "${Green}14.${Font} Copot pemasangan V2Ray"
    echo -e "${Green}15.${Font} Perbarui tugas terjadwal crontab sertifikat"
    echo -e "${Green}16.${Font} Hapus File Sertifikat yang tersisa"
    echo -e "${Green}17.${Font} Tutup \n"
